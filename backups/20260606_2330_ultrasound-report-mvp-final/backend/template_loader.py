...
def search_candidates(text: str, exam_type: str = "", limit: int = 10) -> list[dict]:
    ...
    # 策略0: 正常报告快速检测 — 全文无异常词则直接返回正常模板
    _abnormal_kw = ["结石","囊肿","肌瘤","息肉","增生","钙化","结节","占位","积液",
                    "血栓","狭窄","扩张","反流","关闭不全","畸形","癌","瘤","炎","硬化",
                    "增大","稍大","偏大","饱满","水肿","出血","坏死","梗死","缺血",
                    "回声增强","回声减低","回声不均","无回声","低回声","高回声",
                    "不规则","模糊","渗出","粘连","毛糙","增厚","衰减"]
    _normal_kw = ["未见异常","未见明显异常","未见占位","未见积液","未见结石",
                  "大小正常","形态正常","回声均匀","大小形态正常","位置正常",
                  "走行正常","分布均匀","血管走行清晰","未见扩张","未见肿大",
                  "透声可","壁光滑","未见异常回声","未见明确","正常范围"]
    _has_abnormal = any(kw in text for kw in _abnormal_kw)
    _has_normal = any(kw in text for kw in _normal_kw)
    # 正常报告判断: 有正常描述 + 无任何异常词 + 文本长度>=50
    _is_clearly_normal = _has_normal and not _has_abnormal and len(text) >= 50

    if _is_clearly_normal:
        # 根据检查类型返回对应的正常模板
        _normal_tpl_map = {
            "腹部超声": ("正常男性腹部全套(无门静脉)", "正常女性腹部全套(无门静脉)"),
            "妇产超声": ("子宫附件未见明显异常", "女性经阴道彩超子宫附件"),
            "心脏超声": ("心脏结构未见明显异常", "心脏彩超未见明显异常"),
            "甲状腺超声": ("甲状腺双侧叶未见异常", "甲状腺(正常)"),
            "乳腺超声": ("乳腺未见异常", "乳腺未见异常"),
        }
        _normals = _normal_tpl_map.get(exam_type, ("肝胆胰脾双肾未见明显异常",))
        for nm in _normals:
            if nm in _template_index:
                scored[nm] = max(scored.get(nm, 0), 250)
                break
    ...
    for keyword in sorted(_keyword_index.keys(), key=len, reverse=True):
        if keyword in text and len(keyword) >= 2:
            ...
            scored[name] = max(scored.get(name, 0), 250 + len(keyword) * 3 + extra_bonus)
    ...
    for name in list(_template_index.keys()):
        ...
        if text_organ_categories and name in scored:
            ...
            if not tpl_has_text_organ:
                scored[name] -= 200  # template has NO organ from text → maximum penalty
            elif tpl_categories and not tpl_categories.intersection(text_organ_categories):
                scored[name] -= 180  # template category doesn't match text category → near-max penalty
