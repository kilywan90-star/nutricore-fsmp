...
TEMPLATE_CSV = Path(os.environ.get("TEMPLATE_CSV", "./templates/超声模板.csv"))
...
def _get_manual_map() -> dict:
    ...
    try:
        ...
        m = get_kb().manual_mapping
        if m:
            _manual_cache = m
            return m
    except Exception:
        pass
    _manual_cache = _MANUAL_FALLBACK
    return _MANUAL_FALLBACK

# 向后兼容: 其他模块可能 import MANUAL
MANUAL = _MANUAL_FALLBACK
...
def _search(text: str) -> list[int]:
    ...
    disease_words = ["结石","囊肿","囊","肌瘤","息肉","增生","钙化","血管瘤","脂肪肝","硬化","积水","腹水","畸胎瘤","狭窄","斑块","血栓","结节","积液","肿","腺肌","炎","增大","稍大","偏大","饱满"]
    ...
    for kw, names in _get_manual_map().items():
...
def match_and_fill(raw_text: str, exam_type: str = "") -> dict | None:
    ...
    # P0-1: 上下文过滤 - 跳过"建议:"后的内容避免误提取数值
    ctx_filters = get_rule("extraction.context_filters", {})
    skip_keywords = ctx_filters.get("skip_after_keywords", ["建议", "讨论", "备注", "临床诊断", "处置", "附注"])
    
    filtered_text = raw_text
    for kw in skip_keywords:
        if kw in raw_text:
            # 找到关键词位置,截取之前的内容用于数值提取
            pos = raw_text.find(kw)
            filtered_text = raw_text[:pos].strip()
            break
    
    # P0-2: 自动提取建议段落 (如果存在)
    recommendation = ""
    rec_pattern = ctx_filters.get("recommendation_extract_pattern", r"建议[：:]\s*(.+?)(?:[。；;]|$)")
    rec_match = re.search(rec_pattern, raw_text)
    if rec_match:
        rec_content = rec_match.group(1).strip()
        rec_format = ctx_filters.get("recommendation_format", "建议{content}。")
        recommendation = rec_format.replace("{content}", rec_content)
    
    raw_text = cn_to_arabic(raw_text)  # ASR可能返回中文数字，先转阿拉伯
    _load(); idxs = _search(filtered_text)  # 使用过滤后的文本搜索模板
    if not idxs: idxs = _cat_fallback(filtered_text, exam_type)
    if not idxs: return None

    tpl = _templates[idxs[0]]
    nums = _extract_numbers(filtered_text)  # 从过滤后文本提取数值
    opts = _extract_options(filtered_text, tpl["info1"])
    ...
    return {
        ...
        "study_see": see, "study_hint": hint, "recommendation": recommendation,  # 使用提取的建议
