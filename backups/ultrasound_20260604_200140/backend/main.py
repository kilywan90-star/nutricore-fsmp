...
app = FastAPI(title="超声报告语音结构化", version="2.2.0.20260604.1945")
...
async def structure(req: StructureRequest):
    ...
    corrected = correct_ASR_text(req.text)
    warnings = []

    # === 性别冲突检测 (在匹配前执行) ===
    sex_conflict = detect_sex_conflict(corrected, req.patient_gender)
    if sex_conflict:
        warnings.append(sex_conflict)
        # 将冲突器官替换为标记
        corrected = mask_conflict_organs(corrected, req.patient_gender)

    # === 妊娠词汇检测 (非妇产检查中出现胎心/孕囊等) ===
    pregnancy_conflict = detect_pregnancy_conflict(corrected, req.exam_type, req.patient_gender)
    if pregnancy_conflict:
        warnings.append(pregnancy_conflict)

    # 策略0: 胎儿/产科超声 → 专用固定模板 (仅当非男性时)
    is_fetal_text = any(kw in (req.exam_type + (req.text or "")[:80]) for kw in ["产科","胎儿","四维","排畸","孕","BPD","双顶径","股骨长","胎心"])
    if is_fetal_text and req.patient_gender != "男":
        report = fill_fetal_template(corrected)
        report = _wrap_hints_with_toggle(report)
        report_id = None
        ...
        db.audit_log("rule_extract", patient_id=report_id, input_text=req.text[:200],
                      ...
                      detail={"method":"fetal_template", "confidence": conf, "warnings": warnings})
        return {"success": True, "report": report, "report_id": report_id, "method": "fetal_template", "warnings": warnings}

    # 策略1: 固定模板 + 数值填充 (v2 优化版)
    # 意图预识别: 用患者信息增强匹配
    intent_info = {
        'patient_sex': (req.patient_gender or ''),
        'patient_age': (req.patient_age or 0),
        'clinical_diag': (req.clinical_diag or ''),
    }
    if req.patient_gender or req.patient_age:
        report = match_and_fill_optimized(
            corrected, req.exam_type,
            patient_sex=req.patient_gender or '',
            patient_age=req.patient_age or 0,
            clinical_diag=req.clinical_diag or '',
        )
    else:
        report = match_and_fill(corrected, req.exam_type)
    if report and report.get("_template_matched"):
        ...
        db.audit_log("rule_extract", patient_id=report_id, input_text=req.text[:200],
                      output_text=str(report.get("study_see",""))[:200],
                      detail={"method": report.get("_method", "regex_fill_v2"),
                              "confidence": conf,
                              "engine": "template_engine_v2" if req.patient_gender else "template_filler",
                              "warnings": warnings})
        return {"success": True, "report": report, "report_id": report_id, "method": "regex_fill", "warnings": warnings}
    ...
    return {"success": True, "report": report, "report_id": report_id, "method": report.get("_method", "llm_free"), "warnings": warnings}


# ==================== 性别冲突检测 + 妊娠冲突检测 ====================

FEMALE_ONLY_ORGANS = {"子宫","卵巢","孕囊","宫颈","阴道","输卵管","胎盘","羊水","脐带","胎心","妊娠"}
FEMALE_ONLY_STRICT = {"子宫","卵巢","孕囊","宫颈","阴道","输卵管","胎盘","羊水","脐带","胎心","妊娠","附件","内膜"}
MALE_ONLY_ORGANS = {"前列腺","睾丸","附睾","精索","精囊","阴囊","阴茎"}
...
def detect_pregnancy_conflict(text: str, exam_type: str, patient_gender: str | None) -> str | None:
    """检测妊娠词汇与患者上下文的冲突"""
    pregnancy_kw = {"孕囊","胎心","胎盘","羊水","脐带","早孕","中孕","晚孕","胚芽","胎位","胎儿","脐动脉"}
