...
from template_engine_v2 import match_and_fill_optimized
...
async def structure(req: StructureRequest):
    ...
    # 精确化: 移除"孕"(误匹配早孕"孕囊")和"胎心"(早孕"心管搏动"不含胎儿测量数据)
    ...
    # L0: 输入质量门 — 极短/无实质内容拦截
    import re as _re0
    _meaningful = _re0.sub(r'[\s嗯啊哦呃额呢吧啦噢哦\W]', '', A)
    ...
    # 早孕排除: 仅含孕囊/胚芽/心管搏动但无胎儿测量数据 → 不走胎儿模板
    _early_pregnancy_signs = ("孕囊", "卵黄囊", "胚芽", "心管搏动", "胎芽", "囊性回声", "囊暗区", "妊娠囊")
    _fetal_measurement_signs = ("双顶径", "BPD", "头围", "HC", "腹围", "AC", "股骨长", "FL", "肱骨长")
    _is_early_pregnancy = any(s in (A or "") for s in _early_pregnancy_signs) and \
                          not any(s in (A or "") for s in _fetal_measurement_signs)
    if _is_early_pregnancy:
        is_fetal_text = False
    if is_fetal_text and req.patient_gender != "男":
        ...
        # L2-数值保全: 胎儿路径同样需要校验
        import re as _reF
        _asr_meas = _reF.findall(r'(?:约|大?小约?|长(?:约)?)?\s*\d+(?:\.\d+)?\s*[×xX\*乘]\s*\d+(?:\.\d+)?\s*(?:mm|毫米|cm|厘米)?', A)
        _asr_meas += _reF.findall(r'(?:约|大?小约?|厚约?|长约?|深约?|宽约?|内径约?)\s*\d+(?:\.\d+)?\s*(?:mm|毫米|cm|厘米)', A)
        _asr_meas = list(dict.fromkeys(_asr_meas))
        _plain_f = _reF.sub(r'<[^>]+>', '', report.get("study_see", ""))
        _miss_f = [v for v in _asr_meas if _reF.findall(r'\d+(?:\.\d+)?', _reF.sub(r'[约大小长厚深宽内径]', '', v).strip()) and not any(n in _plain_f for n in _reF.findall(r'\d+(?:\.\d+)?', _reF.sub(r'[约大小长厚深宽内径]', '', v).strip()))]
        if _miss_f:
            _app = '<br><b class="voice">补充测量: ' + '，'.join(_miss_f) + '</b>'
            _fetal_see = report.get("study_see", "")
            _last_div = _fetal_see.rfind('</div>')
            if _last_div >= 0:
                report["study_see"] = _fetal_see[:_last_div] + _app + _fetal_see[_last_div:]
            else:
                report["study_see"] = _fetal_see + _app
            warnings.append(f"数值保全: {len(_miss_f)}个测量值追加到报告末尾")
        report_id = None
        if req.patient_id and req.patient_id.strip():
            try:
                ...
                logging.warning(f"报告保存失败: patient_id={req.patient_id}, error={e}")
    ...
    try:
        ...
        _cv_data = get_kb().cross_validation or {}
    except Exception:
        pass

    # 泛化异常指示词: 短词级别，弥补ABNORMAL_KW长模板短语匹配不到的情况
    _BROAD_ABNORMAL = (
    ...
    def _is_normal_report(text: str) -> bool:
        ...
        if has_abnormal:
            return False
        # 泛化检查: 短词级异常指示词
        if any(kw in text for kw in _BROAD_ABNORMAL):
            return False
        # 方法1: 关键词匹配 (原有逻辑)
        ...
        # 方法2: 交叉验证模式评分 (增强逻辑)
        if _cv_data:
            cv_patterns = [p["pattern"] for p in _cv_data.get("", [])]
            for key, patterns in _cv_data.items():
                if key and key in req.exam_type:
                    cv_patterns.extend(p["pattern"] for p in patterns)
            if cv_patterns:
                match_count = sum(1 for p in cv_patterns if p in text)
                if match_count >= 3:
                    return True
        return False
    ...
    if _is_normal_report(A) and req.exam_type in fast_path_types:
        ...
        # 一致性校验: C路模板结果不能与"正常"分类矛盾
        # 泛化检查: 模板名称 + study_hint诊断 + 结果正文，任一层发现异常即回退
        _tpl_abnormal_indicators = (
            "结石", "增生", "钙化", "稍大", "增大", "偏大", "饱满",
            "息肉", "囊肿", "肌瘤", "积液", "积水", "肿瘤", "占位",
            "结节", "血管瘤", "脂肪肝", "硬化", "狭窄", "血栓", "斑块",
            "炎", "癌", "扩张", "欠均匀", "欠光滑", "毛糙", "增厚",
            "回声增强", "回声减低", "无回声", "低回声", "高回声",
            "不规则", "模糊", "渗出", "粘连",
        )
        _tpl_normal_indicators = ("正常", "未见异常", "未见明显异常")

        tpl_name = C.get("_template_matched", "")
        # 1) 模板名称检查: 名称含异常词且不含正常词 → 异常模板
        tpl_is_normal = any(n in tpl_name for n in _tpl_normal_indicators)
        tpl_is_abnormal = (not tpl_is_normal) and any(ab in tpl_name for ab in _tpl_abnormal_indicators)

        # 2) study_hint诊断检查: hint中含异常词
        hints_text = " ".join(
            h.get("diagnosis", "") if isinstance(h, dict) else str(h)
            for h in C.get("study_hint", [])
        )
        hint_is_abnormal = any(ab in hints_text for ab in _tpl_abnormal_indicators)

        # 3) 结果正文检查: 用ABNORMAL_KW + 异常指示词双重检查
        c_see = C.get("study_see", "")
        c_abnormal_by_kw = any(kw in c_see for kw in ABNORMAL_KW)
        c_abnormal_by_indicator = any(ab in c_see for ab in _tpl_abnormal_indicators if ab not in ("炎",))  # "炎"单字太短，正文误报多
        a_has_abnormal = any(kw in A for kw in ABNORMAL_KW) or any(ab in A for ab in _tpl_abnormal_indicators)

        consistency_fail = False
        fail_reasons = []
        if tpl_is_abnormal:
            consistency_fail = True
            fail_reasons.append(f"模板名异常:{tpl_name}")
        if hint_is_abnormal and not a_has_abnormal:
            consistency_fail = True
            fail_reasons.append(f"hint异常:{hints_text[:80]}")
        if (c_abnormal_by_kw or c_abnormal_by_indicator) and not a_has_abnormal:
            consistency_fail = True
            fail_reasons.append("正文含异常词")

        if consistency_fail:
            logging.warning(f"fast_normal一致性校验失败: 输入正常但C路模板异常, "
                            f"reasons={fail_reasons}, 回退ABCDEF")
            # 不返回，继续走下面的完整 ABCDEF 流水线
        else:
            report = _wrap_hints_with_toggle(C)

            # === fast_normal 后处理: P0-4占位符标记 + L2数值保全 ===
            if report.get("study_see"):
                _fn_see = report["study_see"]
                # P0-4: 占位符标记 (与 abcdef_v3 路径一致)
                _fn_see = _re.sub(
                    r'(?<!class="unfill">)(_{2,}\s*(?:mm|cm|毫米|厘米)?)',
                    r'<i class="unfill">\1</i>', _fn_see
                )
                _fn_see = _re.sub(
                    r'(?<!class="unfill">)([xX]\s*(?:mm|cm)\b)',
                    r'<i class="unfill">\1</i>', _fn_see
                )
                _fn_see = _re.sub(
                    r'(?<!class="unfill">)(未测)',
                ...
                report["study_see"] = _fn_see

                # L2-数值保全: ASR原文数值必须在输出中保留
                _fn_asr_meas = _re.findall(
                    r'(?:约|大?小约?|长(?:约)?)?\s*\d+(?:\.\d+)?\s*[×xX\*乘]\s*\d+(?:\.\d+)?\s*(?:mm|毫米|cm|厘米)?', A
                )
                _fn_asr_single = _re.findall(
                    r'(?:约|大?小约?|厚约?|长约?|深约?|宽约?|内径约?)\s*\d+(?:\.\d+)?\s*(?:mm|毫米|cm|厘米)', A
                )
                _fn_asr_pct = _re.findall(
                    r'(?:约|大?小约?)?\s*\d+(?:\.\d+)?\s*%', A
                )
                _fn_all_vals = list(dict.fromkeys(_fn_asr_meas + _fn_asr_single + _fn_asr_pct))
                _fn_plain = _re.sub(r'<[^>]+>', '', report.get("study_see", ""))
                _fn_missing = []
                for val in _fn_all_vals:
                    _clean_val = _re.sub(r'[约大小长厚深宽内径]', '', val).strip()
                    _num_part = _re.findall(r'\d+(?:\.\d+)?', _clean_val)
                    if _num_part and not any(n in _fn_plain for n in _num_part):
                        _fn_missing.append(val)
                if _fn_missing:
                    _append_html = '<br><b class="voice">补充测量: ' + '，'.join(_fn_missing) + '</b>'
                    report["study_see"] = report.get("study_see", "") + _append_html
                    warnings.append(f"数值保全: {len(_fn_missing)}个测量值追加到报告末尾")
                    logging.info(f"fast_normal L2数值保全: 追加 {_fn_missing}")
    ...
    # ASR质量估算 — 动态路由 (为audit_log提供confidence, 并为后续fast_llm路径做准备)
    from asr_quality_estimator import estimate_asr_quality
    routing = estimate_asr_quality(A, req.exam_type, req.correction_stats)
    logging.info(f"ASR质量估算: confidence={routing.get('confidence', 0):.3f}, route={routing.get('route', 'full')}")
    ...
    # L5: 医疗合规 — 矛盾描述 (从规则引擎加载 + 反义词对增强)
    contradictions = [(c["negative"], c["positive"]) for c in get_rule("validation.contradictions", [])]
    # 添加反义词对矛盾检测
    antonym_pairs = _build_antonym_contradictions(req.exam_type)
    contradictions += [([a], [b]) for a, b in antonym_pairs]
    ...
    range_warnings = validate_numerical_ranges(filled_html)
    for rw in range_warnings:
        validation_issues.append(f"L6: {rw['message']}")
    ...
    # L7: ASR优先覆盖 — 模板输出与ASR矛盾时用B路自由生成替换
    _tpl_normal_phrases = (
        "未见明显异常", "未见明显结节", "未见明显占位", "未见异常回声",
        "未见明显包块", "分布均匀", "形态规则", "大小正常",
        "未见明显异常回声", "未见明显血流异常",
    )
    _asr_abnormal_signs = (
        "结节", "钙化", "增大", "稍大", "低回声", "高回声", "无回声",
        "占位", "包块", "不均匀", "增粗", "衰减", "毛糙", "增厚",
        "斑块", "息肉", "囊肿", "结石", "增生", "饱满",
    )
    _ef_plain = _extract_plain_text(EF.get("filled_study_see_html", ""))
    _tpl_says_normal = any(p in _ef_plain for p in _tpl_normal_phrases)
    _asr_says_abnormal = any(s in A for s in _asr_abnormal_signs)
    if _tpl_says_normal and _asr_says_abnormal:
        # 模板说正常但ASR说有异常 → 用B路自由生成替换
        b_see_html = B.get("study_see", "") if B else ""
        if b_see_html:
            EF["filled_study_see_html"] = b_see_html
            EF["study_hint"] = B.get("study_hint", EF.get("study_hint", []))
            EF["template_name"] = EF.get("template_name", "") + "(ASR覆盖)"
            EF["confidence"] = min(EF.get("confidence", 0.8), 0.65)
            EF["_method"] = "abcdef_v3_asr_override"
            warnings.append("模板输出与ASR矛盾，已用语音内容覆盖")
            logging.info(f"L7 ASR覆盖: 模板说正常但ASR含异常发现")
    ...
    if report.get("_method") != "fetal_template" and report.get("study_see"):
    ...
    # P0-4: 占位符统一标记 — 保留所有占位符供医生手动填写
    # 将各种未填充占位符统一标记为 <i class="unfill"> 格式, 前端橙色显示
    if report.get("study_see") and report.get("_method") != "fetal_template":
        _see = report["study_see"]
        # 统一标记占位符: __ / ___ / x mm / X mm / 未测 → <i class="unfill">
        # 注意: 只标记尚未被P0-3标记的占位符 (避免重复标记)
        _see = _re.sub(
            r'(?<!class="unfill">)(_{2,}\s*(?:mm|cm|毫米|厘米)?)',
            r'<i class="unfill">\1</i>', _see
        )
        _see = _re.sub(
            r'(?<!class="unfill">)([xX]\s*(?:mm|cm)\b)',
            r'<i class="unfill">\1</i>', _see
        ...
        report["study_see"] = _see
        logging.info(f"P0-4 占位符标记: 保留占位符供医生手动填写")

    # P0-5: 去重 — 移除输出中重复的分句 (B路LLM有时会重复生成相同描述)
    if report.get("study_see") and report.get("_method") != "fetal_template":
        _see_dedup = report["study_see"]
        # 按句号/分号/换行/br分割并去重, 保留顺序
        _parts = _re.split(r'((?:。|；|<br\s*/?>|\n|,\s*<br))', _see_dedup)
        _seen_sents = set()
        _deduped = []
        for _p in _parts:
            _p_plain = _re.sub(r'<[^>]+>', '', _p).strip()
            # 只去重长度>8的有意义分句, 避免误删标点/分隔符
            if len(_p_plain) > 8:
                _p_norm = _re.sub(r'\s+', '', _p_plain)
                if _p_norm in _seen_sents:
                    continue  # 跳过重复分句
                _seen_sents.add(_p_norm)
            _deduped.append(_p)
        _deduped_html = ''.join(_deduped)
        if _deduped_html.strip():
            report["study_see"] = _deduped_html

    # L2-数值保全: ASR原文中的测量数值必须全部出现在最终输出中
    _asr_measurements = _re.findall(
        r'(?:约|大?小约?|长(?:约)?)?\s*\d+(?:\.\d+)?\s*[×xX\*乘]\s*\d+(?:\.\d+)?\s*(?:mm|毫米|cm|厘米)?', A
    )
    _asr_single_vals = _re.findall(
        r'(?:约|大?小约?|厚约?|长约?|深约?|宽约?|内径约?)\s*\d+(?:\.\d+)?\s*(?:mm|毫米|cm|厘米)', A
    )
    # 百分比格式 (如 "狭窄约7.3%")
    _asr_pct_vals = _re.findall(
        r'(?:约|大?小约?)?\s*\d+(?:\.\d+)?\s*%', A
    )
    _all_asr_vals = list(dict.fromkeys(_asr_measurements + _asr_single_vals + _asr_pct_vals))  # 去重保序
    _plain_output = _re.sub(r'<[^>]+>', '', report.get("study_see", ""))
    _missing_vals = []
    for val in _all_asr_vals:
        _clean_val = _re.sub(r'[约大小长厚深宽内径]', '', val).strip()
        _num_part = _re.findall(r'\d+(?:\.\d+)?', _clean_val)
        if _num_part and not any(n in _plain_output for n in _num_part):
            _missing_vals.append(val)
    if _missing_vals:
        _append_html = '<br><b class="voice">补充测量: ' + '，'.join(_missing_vals) + '</b>'
        _cur_see = report.get("study_see", "")
        _last_div = _cur_see.rfind('</div>')
        if _last_div >= 0:
            report["study_see"] = _cur_see[:_last_div] + _append_html + _cur_see[_last_div:]
        else:
            report["study_see"] = _cur_see + _append_html
        warnings.append(f"数值保全: {len(_missing_vals)}个测量值追加到报告末尾")
        logging.info(f"L2数值保全: 追加 {_missing_vals}")
    ...
    db.audit_log("abcdef_v3", patient_id=pid, input_text=req.text[:200],
                  ...
                  detail={"template": template_name, "confidence": EF.get("confidence", 0),
                          "route": "full", "asr_confidence": routing.get("confidence", 0)})

    # 将数值范围校验警告加入返回结果
    for rw in range_warnings:
        warnings.append(f"数值异常: {rw['message']}")
    ...
    return {
        ...
        "sources": {
            "A_asr": A[:500],
            "B_free_llm": {"study_see": _extract_plain_text(B.get("study_see",""))[:500], "study_hint": B.get("study_hint",[])} if B else None,
            "C_regex": {"study_see": _extract_plain_text(C.get("study_see",""))[:500], "study_hint": C.get("study_hint",[])} if C else None,
            "D_enhanced": {"study_see": _extract_plain_text(D.get("study_see",""))[:500], "study_hint": D.get("study_hint",[])} if D else None,
            "EF_combined": {"template_name": template_name, "filled": _extract_plain_text(EF.get("filled_study_see_html",""))[:500]},
        },
    }
